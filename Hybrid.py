            ...
            while j<2:
                ...
                if ((j!=0 or i!=0) and (tempi>=0 and tempi<len(tempGrid)) 
                    ...
                    if(checker==False):
                        ...
                        newNode.h3=CalculateManhattanDistance(newGrid)+CalculateHammingDistance(newGrid)
...
def CalculateHammingDistance(matrix):
    ...
    for i in range(4):
        for j in range(4):
            TorF.append(matrix[i][j]==goal[i][j])

    return TorF.count(False)
...
#[[7, 2, 12, 9], [5, 3, 1, 4], [8, 0, 11, 14], [0, 5, 12, 10]]
...
init=Node()
init.graf=intitial
init.h3=CalculateManhattanDistance(init.graf)
